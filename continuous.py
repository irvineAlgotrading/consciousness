...
def update_plot(frame, plot_weights, plot_randomness, self_awareness_aspects, ax1, ax2, input_text):
    ...
    for i, aspect in enumerate(self_awareness_aspects):
        line, = ax1.plot(frame, [w[i] for w in plot_weights[-20:]], label=aspect, lw=3) 
        lines.append(line)
    ...
    if len(plot_randomness) > window_size:
        ...
        ax2.text(0.5, .1, f"Slope: {slope:.4f}", transform=ax2.transAxes, color='black', fontsize=12, ha='center')
